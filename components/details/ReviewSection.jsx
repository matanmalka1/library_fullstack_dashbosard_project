      ...
      <div className="review-section__summary">
        ...
        <div className="review-section__card">
          <div className="review-section__rating">
            <span className="review-section__rating-value">{book.rating || 0}</span>
            <div className="review-section__rating-stars">
              {[1,2,3,4,5].map(i => (
                <Star key={i} className={`review-section__star ${i <= (book.rating || 0) ? 'is-active' : ''}`} />
              ))}
            </div>
            <span className="review-section__rating-count">{approved.length} reviews</span>
          </div>
          {isAuthenticated ? (
            <form onSubmit={handleSubmit} className="review-section__form">
              <div className="review-section__picker">
                {[1,2,3,4,5].map(i => (
                  <button key={i} type="button" onClick={() => setRating(i)} className="review-section__picker-btn">
                    <Star className={`review-section__picker-star ${i <= rating ? 'is-active' : ''}`} />
                  </button>
                ))}
              </div>
              <textarea value={comment} onChange={e => setComment(e.target.value)} className="review-section__textarea" placeholder="Your thoughts..." />
          ...
          <div key={r.id} className="review-section__review">
            <div className="review-section__review-header">
              <div className="review-section__review-user">
                <div className="review-section__review-avatar">{r.userName[0]}</div>
                <div>
                  <h4 className="review-section__review-name">{r.userName}</h4>
                  <div className="review-section__review-stars">
                    {[1,2,3,4,5].map(i => (
                      <Star key={i} className={`review-section__review-star ${i <= r.rating ? 'is-active' : ''}`} />
                    ))}
                  </div>
                </div>
              </div>
              <span className="review-section__review-date">{new Date(r.date).toLocaleDateString()}</span>
            </div>
