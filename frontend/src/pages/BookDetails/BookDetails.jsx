...
export const DetailsPage = () => {
  ...
  const fetchBook = () => {
    if (!id) return;
    bookService.getBookById(id).then((b) => {
      if (b) setBook(b);
      setLoading(false);
    });
  };
  ...
  const handleToggleWishlist = async () => {
    ...
    const next = await wishlistService.toggleWishlist(user.id, id);
  ...
  if (!book)
    return (
      <div className="max-w-[1120px] mx-auto px-4 py-32 text-center">
        <h2 className="font-serif text-2xl md:text-[32px] font-bold mb-4">
          Book Not Found
        </h2>
        <Link
          to="/books"
